...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="${base}/home/css/style.css?v=1.2">
    ...
    <script type="text/javascript">
     	var price = ${product.tgmoney};
     	var num = 1;
     	var mymoney = ${customer.point};
     	$(document).ready(function(){
	     	$(".Spinner").on("click","a",function(){
	     		num = $(".Amount").val();
				var nowmoney=price*num;
				$(".allmoney").html("￥"+nowmoney);
				if(nowmoney>mymoney){
					$("#pointpay").addClass("noenough");
				}else{
					$("#pointpay").removeClass("noenough");
				}
	     	});
     	...
     	function topay(tt){
			...
			$.ajax({
				...
				data:{pid:"${pid!}",gid:"${gid!}",thesize:thesize,num:num,tt:"tuangou",ptype:tt,addressid:addressid},
				type:"post",
    ...
    <div class='container-fluid'>
        <div class="row">
            <div class='pay-addrbox'>
            	<#if address??>
                <a href='address!list.action?tt=tg&tid=${gid!}&pid=${pid!}'>
                    <h5 class="p-adr-lx">${address.name} ${address.phone}</h5>
                    <p class='p-adr-dz'>${address.address}</p>
                    <i class='p-adr-jt'>&#xe609;</i>
                </a>
                <#else>
                <a href='address!list.action?tt=tg&tid=${gid}&pid=${pid!}' class="go-tianj">去添加一个地址</a>
                </#if>
            </div>
            ...
            <div class="pay-pro-inf">
                <div class="p-headbox">
                    ...
                    <div class='p-tit'>
                        <p>${product.name}</p>
                        <span class='p-pic'>价格：￥${product.tgmoney}</span>
                    </div>
                </div>
                <div class='p-set-box'>
                ...
                <div class="p-p-pic">
                    <div class='od-spjg'>
                        ...
                        <span class="od-spjg-2 allmoney">￥${product.tgmoney}</span>
                    </div>
                    <div class='od-spjg'>
            ...
            <div class="m-da-listbox">
                <div class='m-da-list' id="wxpay">
                    <a href="#">
                        <label><i>&#xe627;</i>微信支付</label>
                        <span class="m-da-d"><i>&#xe609;</i></span>
                    </a>
                </div>
                <div class='m-da-list  <#if customer.point lt product.tgmoney>noenough</#if>' id="pointpay">
                    <a href="#">
                        <label><i class="mdl-jf">&#xe625;</i>积分支付<small>（您有${customer.point}积分）<#if customer.point lt product.tgmoney>[余额不足]</#if></small></label>
                        <span class="m-da-d"><i>&#xe609;</i></span>
                    </a>
                </div>
            </div>
