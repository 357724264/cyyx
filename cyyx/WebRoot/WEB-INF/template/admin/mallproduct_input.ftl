...
<link href="${base}/assets/umeditor/themes/default/css/ueditor.css" type="text/css"
	rel="stylesheet">
<link href="${base}/assets/umeditor/themes/default/css/ueditor.min.css" type="text/css"
	rel="stylesheet">
<link href="${base}/assets/umeditor/themes/iframe.css" type="text/css" rel="stylesheet">
<link href="${base}/assets/umeditor/themes/default/dialogbase.css" type="text/css"	rel="stylesheet">
<script type="text/javascript" charset="utf-8"
	src="${base}/assets/umeditor/ueditor.config.js"></script>
<script type="text/javascript" charset="utf-8"
	src="${base}/assets/umeditor/ueditor.all.js"></script>
<script type="text/javascript" charset="utf-8"
	src="${base}/assets/umeditor/ueditor.parse.js"></script>
<!--建议手动加在语言，避免在ie下有时因为加载语言失败导致编辑器加载失败-->
<!--这里加载的语言文件会覆盖你在配置项目里添加的语言类型，比如你在配置项目里配置的是英文，这里加载的中文，那最后就是中文-->
<script type="text/javascript" charset="utf-8"
	src="${base}/assets/umeditor/lang/zh-cn/zh-cn.js"></script>
<!--end编辑框样式+js-->
...
<style>
  ...
  .erweima-box{width:160px; height:160px; box-shadow:1px 1px 2px #ddd,-1px -1px 2px #ddd; padding:10px; position:absolute; top:30px; right:30px;}
  .erweima-box img{width:100%; height:100%;}
</style>
...
<div class="main-content" style="width:1008px; margin:0 auto; margin-top:10px;">
  ...
  <div class="page-header position-relative">
      <h1> <a href="mallproduct!list.action"><i class="icon-laptop" style="font-size:20px"></i>商品管理</a> <small> <i class="icon-double-angle-right"></i> 编辑活动 </small> </h1>
	    <div class="ace-settings-container" id="ace-settings-container"  style=" margin-right:15px; margin-top:-45px; ">
              <div class="btn-group">
                <button class="btn btn-success btn-block" onClick="location.href='mallproduct!list.action'"><i class="icon-cloud-upload"></i>返回上一页</button>
              </div>
            </div>
    </div>
	  <div class="alert alert-block alert-success" style="line-height:30px;">
			<button type="button" class="close" data-dismiss="alert"> <i class="icon-remove"></i> </button>
			<i class="icon-ok green"></i> 温馨提醒：<br/>1、建议商品图尺寸比例为3:5最佳，以458*627px为例，大小不超过300KB。
	  </div>
    ...
    <div id="user-profile-2" class="user-profile row-fluid">
      <div class="tabbable">
          ...
          <div id="home" class="tab-pane in active">
            ...
            <div class="row-fluid">
             ...
             <#assign isEdit=false>
              ...
              <div class="span12" style="margin-top:30px;">
                <form class="form-horizontal" class="mindform" type="post" action="<#if isEdit>json/mallproduct!update.action<#else>json/mallproduct!save.action</#if>" />
            <style>
            .control-label{height:40px;line-height:25px;font-size:16px;}
                ...
                <div class="erweima-box" >
                  <img src="http://admin.wudianyi.com//secure!qrcode.action?bf=http://${(mproduct.systemid)!}.wudianyi.com/mallproduct.action?id=${(mproduct.id)!}" />
                </div>
               
               
               
                <div class="control-group">
                  <label class="control-label" for="cpmc">商品标题：</label>
                  <div class="controls">
                    <input type="text" name="title" id="title" placeholder="" value="${(mproduct.title)!}" class="span5 validate[required]" style="height:40px;line-height:30px;font-size:16px;"/>
                  </div>
                </div>
               ...
               <div class="control-group">
                  ...
                  <div class="controls">
                    <input type="text" name="name" id="name" placeholder="" value="${(mproduct.name)!}" class="span5 validate[required]" style="height:40px;line-height:30px;font-size:16px;"/>
                  </div>
                </div>
               ...
               <div class="control-group">
                	...
                	<div class="controls" style="line-height:40px;">
               			<img class="editable" alt="Alex's Avatar" id="file99-filepreview" src="<#if mproduct??>${mproduct.pic}<#else>${base}/assets/img/cqpic.jpg</#if>"  style="width:229px;height:147px; " />
                		...
                		<input id="file99-pic" type="hidden"  name="pic" value="${(mproduct.pic)!}"/>
                		<br/>（缩略图建议使用11:10图片，正方形亦可，大小不超过300KB。）
                	</div>
                </div>

				<div class="control-group">
                	<label class="control-label" for="title">产品价格：</label>
                	<div class="controls">
                			<input type="text" name="rawmoney" id="rawmoney" placeholder="" value="${(mproduct.rawmoney)!}" class="span4 validate[required]" style="height:40px;line-height:30px;font-size:16px;"/>
                 			元
                	</div>
                </div>
                ...
                <div class="control-group">
                  <label class="control-label" for="title">产品排序：</label>
                  <div class="controls">
                    <input type="text" name="orderById" id="orderById" placeholder="数字越大越靠前" value="${(mproduct.orderById)!}" class="span4 validate[required]" style="height:40px;line-height:30px;font-size:16px;"/>
                  </div>
                </div>
             ...
             <!--  
                <div class="control-group"  style="padding-bottom:10px;">
                    ...
                    <div class="controls"   style="font-size:14px;">
                    	<#if mproduct??&&mproduct.ggs??>
                    		<#list mproduct.ggs as ggg>
                    	...
                    	<input type="hidden" value="${(mproduct.gg)!}" name="gg" id="gg" />
					</div>
				</div>
				--> 
				...
				<div class="control-group"> 
               		<label class="control-label" for="hdjj">产品简介：</label>
                    <div class="controls">
                    	<input type="text" id="intro" name="intro" value="${(mproduct.intro)!}" placeholder="" class="span8" style="height:40px;line-height:30px;font-size:16px;width:500px;" />
                	</div>
                </div>
				
				
            <div class="control-group">
              <label class="control-label" for="productgg">产品参数：</label>
              <div class="controls" style="font-size:14px;">
                <table class="table" id="cpcs-box">
                  <tr>
                    <th class="col-3">参数名</th>
                    <th>参数描述</th>
                    <th></th>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    <td class="c-btnbox"><button type="button" id="add-cpcsbtn">添加参数</button></td>
                    <input type="hidden" value="${(mproduct.quality)!}" name="quality" id="quality" />
                    <input type="hidden" value="${(mproduct.minutia)!}" name="minutia" id="minutia" />
                  </tr>
                </table>
              </div>
            </div>
		<!--+
            <div class="control-group">
              <label class="control-label" for="productgg">是否在首页显示：</label>
              <div class="controls" id="sfsyxs" style="font-size:14px;">
                <label><input type='radio' name="sfsyxs" />是</label>
                <label><input type='radio' name="sfsyxs" checked="checked" />否</label>
              </div>
            </div>
		-->	
					  ...
					  <div class="control-group" style="padding-top:20px;">
							<label class="control-label" for="desc">图文描述：</label>
							<div class="controls">
								 <div class="row-fluid">
									<script type="text/plain" id="mycentent" style="width:1000px;height:240px;">
										<#if mproduct!=null&&mproduct.detail!="">${mproduct.detail}</#if>
									</script>
									<textarea style="display:none" id="txt-content" name="detail" >
										<#if mproduct!=null&&mproduct.detail!="">${mproduct.detail}</#if>
									</textarea>
									<script>
               ...
               <tr></tr>
				...
				<#assign ii=ii+1>
				<div class="control-group">
                	<label class="control-label" for="form-field-1">产品轮询图片：</label>
                	<div class="controls" style="line-height:40px;">
               			<img class="editable" alt="Alex's Avatar" id="file${image_index+1}-filepreview" src="<#if mproduct??><#if image??>${image}<#else>${base}/assets/img/cqproduct.jpg</#if><#else>${base}/assets/img/cqproduct.jpg</#if>"  style="width:355px;height:200px; " />
                		...
                		<br/>（建议产品图尺寸比例为7:4最佳，以710*400px为例，大小不超过300KB。）
                		
                		<span class="shanchu sdel">×</span>
                	</div>
                </div>
                </#list>
                </#if>
...
$(function() {
	//another option is using modals
      ...
      $(".sdel").click(function() {
    	...
    	var cont1='${(product.instruction)!}'.replace(/<br>/g,"\n");
      $('#instruction').text(cont1);
...
function fileupload(id){
	...
	$("form").validationEngine();
		<#if mproduct??&&mproduct.ggs??>
			var ggnum = ${mproduct.ggs?size};
		<#else>
			var ggnum = 1;
		</#if>
		
		
		<#if mproduct??&&mproduct.qualityList??>
			var qnum = ${mproduct.qualityList?size};
		<#else>
			var qnum = 1;
		</#if>
		
		
		<#if mproduct??&&mproduct.minutiaList??>
			var mnum = ${mproduct.minutiaList?size};
		<#else>
			var mnum = 1;
		</#if>
