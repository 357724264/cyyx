...
function page(){
	...
	ss = $("#pstores li.selected").attr("data-value");
	bb = $("#pbarnds li.selected").attr("data-value");
	type = $("#pclazzs li.selected").attr("data-value");
	min = $("#start_price").val();
	max = $("#end_price").val();
	$.ajax({
		url:"agentproduct!jsonlist.action",
		data:{ss:ss,bb:bb,type:type,pn:pn,min:min,max:max,sort:sort,ord:ord,keyword:keyword,level:level},
...
$(document).ready(function(){
	...
	$(".pt").bind("click",function(event){
		...
		//$.ajax({
			//url:"category!down.action?cid="+type,
			//success:function(html){
				//var dat = eval("("+html+")")
