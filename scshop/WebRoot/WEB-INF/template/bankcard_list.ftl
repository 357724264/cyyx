...
<body class="add-bankpadd">
		...
		<dl class="bankbox">
			...
			<dd id="dd_${bankcard_index}">
				<div class="xzbank-head">
					<div class="xzb-lb">
						<label>
							<input type="radio" name="bank" <#if bankcard.isDefault==1>checked="checked"</#if>>
							<i class="iconfont icon-duigou"></i>默认银行卡
						</label>
					</div>
					<div class="xzb-btnbox">
						<a href="bankcard!edit.action?id=${bankcard.id}" class="edit"><i class="iconfont icon-bianji3"></i>编辑</a>
						<a href="#" data-id="dd_${bankcard_index}" class="delt"><i class="iconfont icon-shanchu1"></i>删除</a>
					</div>
				</div>
				<div class="xzb-cont">
					<p>
	...
	<div class="add-bankbox">
		<a href='bankcard!edit.action'>+&nbsp;添加银行卡</a>
	</div>
