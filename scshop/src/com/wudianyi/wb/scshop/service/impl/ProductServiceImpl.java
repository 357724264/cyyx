...
public class ProductServiceImpl extends BaseServiceImpl<Product, Integer>
	...
	@Override
	public void updateProductPromotionsInfo(int productid, SubProduct subProduct) {
		...
		if (!StringUtils.isEmpty(product.getPromotionsjson())) {
			...
			if (has) {
				...
				product.setSeckillnum(product.getSeckillnum()-1);
			//如果全部可购买数都为0，则初始化商品促销信息
			}else {
				product.setSeckillnum(0);
				product.setProductType(0);
				product.setPromotionsjson(null);
			}
