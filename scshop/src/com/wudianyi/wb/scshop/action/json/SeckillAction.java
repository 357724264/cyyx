...
public class SeckillAction extends BaseAction {
	...
	public String order() {
		...
		// 看是否已经有未支付的订单
		Order checkorder = orderService.get(
				new QueryParam(5).add("userid", userid).add("del", 0)
						.add("promotions", 1).add("payStat", 0)
						.add("subpromotionproductid", id), false);
		if (checkorder != null) {
			if (checkorder.getStatOverdueTime() > new Date().getTime()) {
